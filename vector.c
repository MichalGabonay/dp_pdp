...
void insert_to_index(Vector *vector, int index, int value) {
    int begin_size = vector->size;
    if (vector->size >= vector->capacity)
    {
        resize(vector);
    }
    
    for (int i = begin_size - 1; i >= index; i--)
    {
        set(vector, i+1, get(vector, i));
    }
...
void swap_vector_values(Vector *vector, int position1, int position2) {
    int value1 = get(vector, position1);
    set(vector, position1, get(vector, position2));
    set(vector, position2, value1);
}